...
from .models import EdgeModel, SRModel, GradientModel
...
class GradientMatch():
    ...
    def train(self):
        ...
        while(keep_training):
            ...
            for items in train_loader:
                ...
                lr_images, hr_images, lr_gradients, hr_gradients = self.cuda(*items)

                # gradient model
                if model == 1:
                    # train
                    hr_gradients_pred, gen_loss, dis_loss, logs = self.gradient_model.process(lr_images, hr_images, lr_gradients, hr_gradients)

                    # metrics
                    precision, recall = self.edgeacc(hr_gradients, hr_gradients_pred)
                    logs.append(('precision', precision.item()))
                    logs.append(('recall', recall.item()))

                    # backward
                    self.gradient_model.backward(gen_loss, dis_loss)
                ...
                # sr model / joint model
                    ...
                    # train
                    hr_gradients_pred = self.scale(lr_gradients) if model == 2 else self.gradient_model.forward(lr_images, lr_gradients).detach()
                    hr_images_pred, gen_loss, dis_loss, logs = self.sr_model.process(lr_images, hr_images, lr_gradients, hr_gradients_pred)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for items in test_loader:
                ...
                #gradient model
                if model == 1:
                    outputs = self.gradient_model.forward(lr_images, lr_gradients)

                # sr model / joint model
                else:
                    hr_gradients_pred = self.scale(lr_gradients) if model == 2 else self.gradient_model.forward(lr_images, lr_gradients).detach()
                    outputs = self.sr_model(lr_images, hr_gradients_pred)
    ...
    def sample(self):
        ...
        # sr model / joint model
            ...
            iteration = self.sr_model.iteration
            hr_gradients = self.scale(lr_gradients) if model == 2 else self.gradient_model.forward(lr_images, lr_gradients).detach()
            outputs = self.sr_model(lr_images, hr_gradients)
